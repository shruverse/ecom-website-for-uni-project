...
$stmt = $conn->prepare("SELECT COUNT(*) AS total_records FROM products WHERE product_category IN ('jewellery')");
$stmt->execute();
...
$stmt1 = $conn->prepare("SELECT * FROM products WHERE product_category IN ('jewellery') LIMIT $offset,$total_records_per_page");
$stmt1->execute();
